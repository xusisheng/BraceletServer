 ...
 */
package com.aw.bracelet.huawei.utils;


/** 1.5.  订阅平台业务数据通知
	客户应用服务器与平台对接，需要通过订阅平台的消息通知类型获取。客户通过程序调用平台订阅接口，选择通知类型，添加回调地址（客户应用程序开发的接收数据接口），当设备上报数据到平台之后，平台会调用客户添加的回调地址，向该地址推送数据。
	1）平台订阅接口
	2）订阅接口请求参数中的notifyType和callbackUrl为必填项，如选择notifyType为deviceDataChanged，当设备上报数据到平台之后，平台就会调用客户的callbackUrl接口地址推送数据。
	回调地址可以选用https或者http协议。
*/
...
//    public static final String BASE_URL = "https://device.api.ct10649.com:8743";
    ...
    public static final String APPID = "FAsFKM6Yi8eHnjl2ZJj2S4_PV8Ua";
    //生产环境：正式平台企业门户登录地址：https://device.local.ct10649.com:8843 应用ID: zK7mNPm6ZYxwC5TOSjlZRLgc_uQa
    public static final String SECRET = "DpylMllxykAAD6eW30fAAMfsBDEa";
    ...
    /** 阿里云后台地址*/
    public static final String CALLBACK_BASE_URL = "http://47.111.30.104:88/uploadData";  
     ...
     * 2. REFRESH_TOKEN
     */
    //https://180.101.147.89:8743/iocm/app/sec/v1.1.0/login
